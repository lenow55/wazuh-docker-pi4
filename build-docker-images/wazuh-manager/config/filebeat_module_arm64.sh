...
curl -L -O https://artifacts.elastic.co/downloads/beats/filebeat/${FILEBEAT_CHANNEL}-${FILEBEAT_VERSION}-aarch64.rpm &&
  yum install -y ${FILEBEAT_CHANNEL}-${FILEBEAT_VERSION}-aarch64.rpm && rm -f ${FILEBEAT_CHANNEL}-${FILEBEAT_VERSION}-aarch64.rpm &&
